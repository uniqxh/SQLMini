...
void CBPlusTree::printData(int num) const
{
         ...
         { 
             ...
             case EQ: 
                 { 
                     ...
                     { 
						 int index=result.keyIndex;
						 while(result.targetNode->getKeyValue(index)==compareKey)
						 {
                         results.push_back(result.targetNode->getData(index)); 
						 index++;
						 }
                     } 
                 } 
 ...
 void CBPlusTree::printAll(string name) 
 {
	 const char *aa;
	 aa=name.c_str();
	freopen(aa,"w",stdout);
	...
	{ 
		for (i=0; i<itr->getKeyNum(); ++i) 
		{ 
			cout<<itr->getKeyValue(i)<<" "<<itr->getData(i)<<endl;
		} 
		itr = itr->getRightSibling(); 
	} 
 }
 ...
 { 
     ...
     { 
          recursive_search(((CInternalNode*)pNode)->getChild(childIndex), key, result); 
		  return;
     } 
 } 